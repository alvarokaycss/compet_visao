...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"\n[OK] Extracao concluida: {meta['frames_processed']} quadros"
        f"\n  Arquivo: {config.output_path}"
        f"\n  Deteccao de maos:  {meta['hand_detection_rate'] * 100:.1f}% dos quadros"
        f"\n  Deteccao de pose:  {meta['pose_detection_rate'] * 100:.1f}% dos quadros"
    )
    if meta["hand_detection_rate"] < 0.90:
        print(
            "  [AVISO] Deteccao de maos abaixo da meta de 90% (M2 do PRD) - verifique "
            "iluminacao, enquadramento e qualidade do video (ver protocolo de captura)."
        )
    return 0
